...
public class smsSender {
	...
	private boolean SMSC_POST    = false;
	...
	public smsSender(String login, String password, String charset, boolean debug) {
		SMSC_LOGIN    = login;
		SMSC_PASSWORD = password;
		SMSC_CHARSET  = charset;
		SMSC_DEBUG    = debug;
	...
	public String[] sendSms(String phones, String message, int translit, String time, String id, int format, String sender, String query) {
		...
		if (SMSC_DEBUG) {
			if (Integer.parseInt(m[1]) > 0) {
				System.out.println("Success. ID: " + m[0] + ", total SMS: " + m[1] + ", price: " + m[2] + ", balance: " + m[3]);
			}
			else {
	...
	public String[] getSmsCost(String phones, String message, int translit, int format, String sender, String query){
		...
		if (SMSC_DEBUG) {
			if (Integer.parseInt(m[1]) > 0) {
				System.out.println("Spam cost: " + m[0] + ", Total sms: " + m[1]);
			}
			else {
				System.out.print("Error №" + Math.abs(Integer.parseInt(m[1])));
			}
